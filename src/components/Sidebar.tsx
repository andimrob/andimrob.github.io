...
function SocialLink({
  ...
  return (
    <a
      ...
      aria-label={label}
      className="flex h-10 w-10 items-center justify-center text-gray-400 transition-colors hover:text-primary dark:text-gray-500 dark:hover:text-primary"
    >
      <svg
        viewBox="0 0 24 24"
        fill="currentColor"
...
function Sidebar() {
  ...
  return (
    <aside className="px-6 pt-30 pb-12 lg:sticky lg:top-0 lg:flex lg:h-screen lg:w-1/2 lg:flex-col lg:justify-between lg:py-20">
      <div>
        {/* Name */}
                ...
                >
                  <span
                    className={`h-px transition-all ${
                      active === s.id
                        ? "w-12 bg-gray-900 dark:bg-white dark:shadow-[0_0_8px_rgba(255,255,255,0.3)]"
                        : "w-6 bg-gray-300 group-hover:w-10 group-hover:bg-gray-500 dark:bg-gray-600 dark:group-hover:bg-gray-400"
                    }`}
                  />
                  {s.label}
                </a>
      ...
      {/* Bottom section: social links + nametag */}
      <div className="mt-8 lg:mt-0">
        ...
        <div className="flex items-center gap-5">
          ...
          {/* Theme toggle — desktop only (mobile has it in the prism nav) */}
          <button
            onClick={toggle}
            aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
            className="hidden lg:flex h-10 w-10 items-center justify-center rounded-full border-2 border-gray-200 text-gray-500 transition-colors hover:border-primary hover:text-primary dark:border-gray-700 dark:text-gray-400 dark:hover:border-primary dark:hover:text-primary"
          >
            {theme === "dark" ? (
